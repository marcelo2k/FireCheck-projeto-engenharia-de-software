...
let itensOrcamentoRequest = []; // Array para itens do orçamento atual
let servicosCatalogo = []; // Cache dos serviços (DTOs)
...
async function handleFetchResponse(response, expectJson = true) {
    ...
    if (!text && expectJson) return []; // Retorna ARRAY vazio se resposta vazia mas esperava JSON
    if (!text && !expectJson) return "";
    if (expectJson) {
        try {
             if (text.trim() === '[]') return []; // Corrige parse para array vazio
...
formUsuario.addEventListener('submit', async (e) => {
    ...
    } catch (error) { alert(`Erro ao conectar para ${method === 'PUT' ? 'atualizar' : 'cadastrar'} usuário: ` + error.message); console.error("Fetch Error:", error); }
});
...
formCliente.addEventListener('submit', async (e) => {
    ...
    } catch (error) { alert(`Erro ao conectar para ${method === 'PUT' ? 'atualizar' : 'cadastrar'} cliente: ` + error.message); console.error(error); }
});
...
async function popularDropdownClientes() {
     try {
        const response = await fetch(API_URL_CLIENTES); const clientes = await handleFetchResponse(response);
        console.log("Clientes recebidos (dropdown Edificacao):", clientes);
    ...
    } catch (error) { console.error('Erro CRÍTICO ao buscar/popular clientes (Edificacao):', error); if (selectClienteEdificacao) selectClienteEdificacao.innerHTML = '<option value="">Erro!</option>';}
}
...
formEdificacao.addEventListener('submit', async (e) => {
    ...
    } catch (error) { alert(`Erro ao conectar para ${method === 'PUT' ? 'atualizar' : 'cadastrar'} edificação: ` + error.message); console.error(error); }
});
...
async function listarEdificacoes() {
    try {
        ...
        console.log("Edificacoes recebidas:", edificacoes);
...
// --- DEFINIÇÃO CORRETA DA FUNÇÃO popularDropdownEdificacoesParaEquipamento ---
...
formEquipamento.addEventListener('submit', async (e) => {
    ...
    } catch (error) { alert(`Erro ao ${method === 'PUT' ? 'atualizar' : 'cadastrar'} equipamento: ` + error.message); console.error(error); }
});
...
// --- DEFINIÇÃO CORRETA DA FUNÇÃO popularDropdownEdificacoesParaInspecao ---
...
// --- DEFINIÇÃO CORRETA DA FUNÇÃO popularDropdownTecnicos ---
...
formInspecao.addEventListener('submit', async (e) => {
    ...
    } catch (error) { alert(`Erro ao conectar para ${method === 'PUT' ? 'atualizar' : 'agendar'} inspeção: ` + error.message); console.error(error); }
});
...
// --- DEFINIÇÃO CORRETA DA FUNÇÃO popularDropdownInspecoes ---
...
formItemInspecionado.addEventListener('submit', async (e) => {
    ...
    if (itemInspecEditando) {
        ...
        method = 'PUT';
        delete payload.idInspecao; delete payload.idEquipamento; // IDs vão na URL
    }
    try {
        const response = await fetch(url, { method: method, headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
    ...
    } catch (error) { alert(`Erro ao conectar para ${method === 'PUT' ? 'atualizar' : 'salvar'} item: ` + error.message); console.error(error); }
});
...
formNaoConformidade.addEventListener('submit', async (e) => {
    ...
    } catch (error) { alert(`Erro ao conectar para ${method === 'PUT' ? 'atualizar' : 'registrar'} NC: ` + error.message); console.error(error); }
});
...
formServico.addEventListener('submit', async (e) => {
    ...
    if (!servicoPayload.nome || !servicoPayload.valorUnitario) { alert("Nome e Valor são obrigatórios."); return; }
    const url = idServicoEditando ? `${API_URL_SERVICOS}/${idServicoEditando}` : API_URL_SERVICOS;
    const method = idServicoEditando ? 'PUT' : 'POST';
    try {
        const response = await fetch(url, { method: method, headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(servicoPayload) });
    ...
    } catch (error) { alert(`Erro ao conectar para ${method === 'PUT' ? 'atualizar' : 'cadastrar'} serviço: ` + error.message); console.error(error); }
});

// --- DEFINIÇÃO CORRETA DA FUNÇÃO listarServicos ---
...
// --- DEFINIÇÃO CORRETA DA FUNÇÃO popularDropdownEdificacoesParaOrcamento ---
...
// --- DEFINIÇÃO CORRETA DA FUNÇÃO popularDropdownTecnicosParaOrcamento ---
...
// --- DEFINIÇÃO CORRETA DA FUNÇÃO popularDropdownServicos ---
...
// --- DEFINIÇÃO CORRETA DA FUNÇÃO popularDropdownOrcamentosPendentes ---
...
async function atualizarStatusOS(id, novoStatus) {
     if (!novoStatus) { listarOrdensServico(); return; } // Reseta select se clicou em "Mudar Status..."
